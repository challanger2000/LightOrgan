...
std::vector<std::uint8_t> makeClickWav()
{
    ...
    constexpr std::uint32_t frames = 1543;
    constexpr std::uint32_t dataBytes = frames * 2;
    std::vector<std::uint8_t> wav(44 + dataBytes, 0);
    auto put16 = [&](std::size_t p, std::uint16_t v) { wav[p]=static_cast<std::uint8_t>(v&0xff); wav[p+1]=static_cast<std::uint8_t>((v>>8)&0xff); };
    auto put32 = [&](std::size_t p, std::uint32_t v) { for(int i=0;i<4;++i) wav[p+i]=static_cast<std::uint8_t>((v>>(8*i))&0xff); };
    std::memcpy(wav.data(),"RIFF",4); put32(4,36+dataBytes);
    std::memcpy(wav.data()+8,"WAVEfmt ",8); put32(16,16); put16(20,1);
    put16(22,channels); put32(24,sampleRate); put32(28,sampleRate*2); put16(32,2); put16(34,bits);
    std::memcpy(wav.data()+36,"data",4); put32(40,dataBytes);
    ...
    for(std::uint32_t i=0;i<frames;++i){
        ...
        const double t2=t>0.018?t-0.018:0.0;
...
void playMechanicalButtonClick()
{
    ...
    PlaySoundA(reinterpret_cast<LPCSTR>(click.data()),nullptr,SND_MEMORY|SND_ASYNC|SND_NODEFAULT|SND_NOSTOP);
#endif
}
